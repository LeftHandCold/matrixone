...
func (u *tokenizeState) start(tf *TableFunction, proc *process.Process, nthRow int, analyzer process.Analyzer) error {
	...
	values := make([]fulltext.IndexValue, 0, vlen-1)
	for i := 1; i < vlen; i++ {
		values = append(values, fulltext.IndexValue{
			Text: tf.ctr.argVecs[i].GetStringAt(nthRow),
			Raw:  tf.ctr.argVecs[i].GetRawBytesAt(nthRow),
			Type: types.T(tf.Args[i].Typ.Id),
		})
	}
	tokens, err := fulltext.TokenizeIndexValues(u.param, values, func(link string) (string, error) {
		dl, err := datalink.NewDatalink(link, proc)
		if err != nil {
			return "", err
		}
		data, err := dl.GetPlainText(proc)
		if err != nil {
			return "", err
		}
	...
	if len(tokens) > 0 {
		u.doc.Words = append(u.doc.Words, FullTextEntry{DocId: id, Word: fulltext.DOC_LEN_WORD, Pos: int32(len(tokens))})
	}
