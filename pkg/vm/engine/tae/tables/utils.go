...
func LoadPersistedColumnData(
	...
	bat, err := blockio.LoadColumns(ctx, []uint16{uint16(def.SeqNum)}, []types.Type{def.Type}, fs.Service, location, nil, accountId)
	if err != nil {
		return
	}
...
func LoadPersistedDeletes(
	...
	movbat, err := blockio.LoadColumns(ctx, []uint16{0, 1, 2, 3}, nil, fs.Service, location, nil, accountId)
	if err != nil {
		return
	}
