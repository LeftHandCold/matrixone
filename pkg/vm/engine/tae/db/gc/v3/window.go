...
func (w *GCWindow) ScanCheckpoints(
	...
	getOneBatch := func(cxt context.Context, bat *batch.Batch, mp *mpool.MPool) (bool, error) {
		...
		reader, err := getCkpReader(ctx, checkpointEntries[0])
		if err != nil {
		...
		if processCkpData != nil {
			if err = processCkpData(checkpointEntries[0], reader); err != nil {
				if !moerr.IsMoErrCode(err, moerr.ErrFileNotFound) {
					return false, err
				}
		...
		collectObjectsFromCheckpointData(ctx, reader, objects)
...
func collectObjectsFromCheckpointData(ctx context.Context, reader *logtail.CKPReader, objects map[string]*ObjectEntry) {
	reader.ForEachRow(
		ctx,
		func(
			account uint32,
