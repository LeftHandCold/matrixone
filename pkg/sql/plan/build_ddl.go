...
func buildFullTextIndexTableV1(createTable *plan.CreateTable, indexInfos []*tree.FullTextIndex, colMap map[string]*ColDef, pkeyName string, ctx CompilerContext) error {
	for _, indexInfo := range indexInfos {
		...
		tableDef := &TableDef{Name: indexTableName, TableType: catalog.FullTextIndex_TblType}
		...
		properties := []*plan.Property{{Key: catalog.SystemRelAttr_Kind, Value: catalog.FullTextIndex_TblType}}
		tableDef.Defs = append(tableDef.Defs, &plan.TableDef_DefType{
			Def: &plan.TableDef_DefType_Properties{
