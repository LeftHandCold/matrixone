...
func BlockRead(
	...
	// read
	columnBatch, err := BlockReadInner2(
...
func BlockReadInner2(
	...
	columnBatch, err := readBlockData2(ctx, colIdxes, colTypes, info, ts,
		fs, pool)
	if err != nil {
		return nil, err
	}
	if info.DeltaLoc != "" {
		deleteBatch, err := readBlockDelete2(ctx, info.DeltaLoc, fs)
		if err != nil {
			return nil, err
		}
		recordDeletes2(columnBatch, deleteBatch, ts)
		logutil.Infof(
...
func readBlockData2(ctx context.Context, colIndexes []uint16,
	colTypes []types.Type, info *pkgcatalog.BlockInfo, ts types.TS,
...
func readBlockDelete2(ctx context.Context, deltaloc string, fs fileservice.FileService) (*containers.Batch, error) {
	bat := containers.NewBatch()
...
func recordDeletes2(columnBatch *containers.Batch, deleteBatch *containers.Batch, ts types.TS) {
	if deleteBatch == nil {
		return
	}
	// record visible delete rows
