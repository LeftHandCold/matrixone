...
func BuildColumnIndex(writer objectio.Writer, block objectio.BlockObject, colDef *catalog.ColDef, columnData containers.Vector, isPk, isSorted bool) (metas []blockio.IndexMeta, err error) {
	zmPos := 0
...
func BuildBlockIndex(writer objectio.Writer, block objectio.BlockObject, schema *catalog.Schema, columnsData *containers.Batch, isSorted bool) (err error) {
	blkMetas := blockio.NewEmptyIndicesMeta()
	pkIdx := -10086
	if schema.HasPK() {
