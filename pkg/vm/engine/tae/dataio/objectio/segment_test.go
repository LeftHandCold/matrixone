...
func TestSegmentFile_Replay(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		assert.Nil(t, err)
		name := EncodeDeleteNameWithVersion(block.(*blockFile).id, blockTs, nil)
		file, err := block.(*blockFile).seg.fs.OpenFile(name, os.O_RDWR)
		assert.Nil(t, err)
		assert.Equal(t, int64(len(deletesBuf)), file.Stat().OriginSize())
	...
	for i := 0; i < 1; i++ {
		...
		ts, err := block.ReadTS()
		assert.Nil(t, err)
		name := EncodeDeleteNameWithVersion(block.(*blockFile).id, ts, nil)
		file, err := block.(*blockFile).seg.fs.OpenFile(name, os.O_RDWR)
		assert.Nil(t, err)
		assert.Equal(t, int64(len(deletesBuf)), file.Stat().OriginSize())
		colBlk0, err := block.OpenColumn(0)
	...
	assert.Equal(t, 1, len(seg.(*segmentFile).fs.(*ObjectFS).nodes))

	segReplay := SegmentFactory.Build(dir, id)
	assert.Equal(t, 1, len(segReplay.(*segmentFile).fs.(*ObjectFS).nodes))
}
