...
func ListSnapshotCheckpointWithMeta(
	...
	// If snapshot ts equals maxGlobalEnd, we need to return only the global checkpoint
	// This handles the case where FilterSortedMetaFilesByTimestamp returns files including gckp
	...
	// Original logic for other cases (ts < gckp.end)
	// FilterSortedMetaFilesByTimestamp returns files without gckp in this case,
	// so the original logic still works correctly
	for i := range entries {
		p := maxGlobalEnd.Prev()
