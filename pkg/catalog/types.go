...
// IsLifecycleRestoreStagingTable reports whether name has the exact canonical
// shape generated by Lifecycle Restore: the internal prefix followed by a
// hyphen-free UUID. It deliberately remains separate from IsHiddenTable:
// Restore staging is an ordinary TAE table whose user access is rejected by
// the planner while the internal Restore executor retains normal
// CREATE/INSERT/RENAME/DROP behaviour.
func IsLifecycleRestoreStagingTable(name string) bool {
	if len(name) != len(LifecycleRestoreTableNamePrefix)+LifecycleRestoreTableIDSuffixLength ||
		!strings.EqualFold(name[:len(LifecycleRestoreTableNamePrefix)], LifecycleRestoreTableNamePrefix) {
		return false
	}
	for _, value := range name[len(LifecycleRestoreTableNamePrefix):] {
		if !((value >= '0' && value <= '9') ||
			(value >= 'a' && value <= 'f') ||
			(value >= 'A' && value <= 'F')) {
			return false
...
const (
	...
	LifecycleRestoreTableNamePrefix       = "__mo_lifecycle_restore_"
	LifecycleRestoreTableIDSuffixLength   = 32
	LifecycleRestoreTableSQLRegexpPattern = `^__mo_lifecycle_restore_[0-9a-f]{32}$`
