...
func foldVarExprsInExpr(expr *plan.Expr, proc *process.Process) (bool, error) {
	if expr == nil {
		return false, nil
	}
	if _, ok := expr.Expr.(*plan.Expr_V); ok {
...
func foldVarExprsInValue(v reflect.Value, seen map[uintptr]struct{}, proc *process.Process) (bool, error) {
	...
	if v.Kind() == reflect.Pointer {
		...
		if v.Type() == planExprPtrType {
			return foldVarExprsInExpr(v.Interface().(*plan.Expr), proc)
		}
	...
	case reflect.Map:
		iter := v.MapRange()
		for iter.Next() {
			keyFolded, err := foldVarExprsInValue(iter.Key(), seen, proc)
			if err != nil {
				return false, err
			}
			valueFolded, err := foldVarExprsInValue(iter.Value(), seen, proc)
			if err != nil {
				return false, err
			}
			folded = folded || keyFolded || valueFolded
		}
	case reflect.Struct:
		if v.Type() == operatorBaseType || v.Type() == operatorBasePType {
			return false, nil
...
func foldVarExprsInExpressionGetters(value any, proc *process.Process) (bool, error) {
	...
	if getter, ok := value.(argExpressionsGetter); ok {
		for _, expr := range getter.GetArgExpressions() {
			exprFolded, err := foldVarExprsInExpr(expr, proc)
			if err != nil {
				return false, err
			}
			folded = folded || exprFolded
		}
	}
	if getter, ok := value.(lockRowsExpressionsGetter); ok {
		for _, expr := range getter.GetLockRowsExpressions() {
			exprFolded, err := foldVarExprsInExpr(expr, proc)
			if err != nil {
				return false, err
			}
