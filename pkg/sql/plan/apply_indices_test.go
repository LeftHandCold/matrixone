...
func TestApplyIndicesForProjectSkipsUnguardedNestedFullTextRewrite(t *testing.T) {
	builder := NewQueryBuilder(planpb.Query_SELECT, NewMockCompilerContext(true), false, true)
	...
	builder.prepareSpecialIndexGuards(1)
	defer builder.resetSpecialIndexGuards()

	nodeID, err := builder.applyIndicesForProject(3, innerProj, map[[2]int32]int{}, map[[2]int32]*planpb.Expr{})
	require.NoError(t, err)
	require.Equal(t, int32(3), nodeID)
	require.Equal(t, []int32{4}, innerProj.Children)
	require.NotEqual(t, "fulltext_match_score", innerProj.ProjectList[1].GetF().GetFunc().GetObjName())
}
