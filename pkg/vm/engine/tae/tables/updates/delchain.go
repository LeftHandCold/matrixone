...
func (chain *DeleteChain) HasDeleteIntentsPreparedInLocked(from, to types.TS) (found bool) {
	chain.LoopChain(func(vn txnif.MVCCNode) bool {
		...
		if found {
			logutil.Infof("HasDeleteIntentsPreparedInLocked %v, n: %v, from %v, rows: %v", chain.mvcc.meta.StringLocked(), n.Prepare.ToString(), from.ToString(), n.mask.String())
		}
		if n.IsActive() {
			return true
		}
		...
		found, _ = n.PreparedIn(from, to)
		if found {
			logutil.Infof("PreparedIn %v, n: %v, from %v, rows: %v", chain.mvcc.meta.StringLocked(), n.Prepare.ToString(), from.ToString(), n.mask.String())
		}
		if n.IsAborted() {
			found = false
		}
