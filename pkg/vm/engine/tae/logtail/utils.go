...
func (collector *BaseCollector) VisitSeg(entry *catalog.SegmentEntry) (err error) {
	...
	for _, node := range mvccNodes {
		...
		if segNode.HasDropCommitted() {
			...
			collector.data.bats[SEGDeleteIDX].GetVectorByName(catalog.AttrCommitTs).Append(entry.GetTable().ID, false)
	...
	for _, node := range mvccNodes {
		...
		if metaNode.BaseNode.MetaLoc.IsEmpty() || metaNode.Aborted {
			...
			if metaNode.HasDropCommitted() {
				...
				collector.data.bats[BLKMetaDeleteIDX].GetVectorByName(SnapshotAttr_TID).Append(entry.GetSegment().GetTable().ID, false)
				...
				collector.data.bats[BLKCNMetaInsertIDX].GetVectorByName(SnapshotAttr_TID).Append(entry.GetSegment().GetTable().ID, false)
			} else {
				...
				collector.data.bats[BLKMetaInsertIDX].GetVectorByName(SnapshotAttr_TID).Append(entry.GetSegment().GetTable().ID, false)
